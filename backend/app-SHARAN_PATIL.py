...
def handle_process_frame(data):
    ...
    try:
        ...
        if filtered:
            ...
            top_obj = sorted_dets[0]
            if top_obj.get('urgency') == 'critical':
                message = f"STOP! {top_obj['name']} ahead!"
                urgency = 'critical'
                vibrate_pattern = [200, 100, 200]
                pan, volume = compute_pan_volume(top_obj['position_x'], top_obj['distance'])
            elif top_obj.get('urgency') == 'warning':
                message = f"Caution, {top_obj['name']} nearby."
                urgency = 'warning'
                vibrate_pattern = [100]
                pan, volume = compute_pan_volume(top_obj['position_x'], top_obj['distance'])
            else:
...
def handle_get_navigation(data):
    ...
    Expects: {'start': 'Optional Start Name', 'end': 'Destination Name'}
    If start is missing or 'current', defaults to 'Entrance'.
    """
    start_point = data.get('start', 'Entrance')
    if start_point.lower() == 'current':
        start_point = 'Entrance' # Force Entrance as start per user requirement
        
    end_point = data.get('end')
    
    print(f"📍 Navigation Request: {start_point} -> {end_point}")
    
    if not end_point:
        emit('navigation_response', {'error': 'Destination not specified.'})
        return

    instructions = navigator.find_shortest_path(start_point, end_point)
    
    if instructions:
        # Extract full route coordinates for drawing on map
        route_coords = [list(reversed(inst['coords'])) for inst in instructions] # [Lat, Lon] for Leaflet
        # Also need path from start to first instruction? 
        # Actually navigator.find_shortest_path returns instructions with the target node of each step.
        # We might want the full path of nodes for smooth drawing.
        # For now, let's trust instructions' coords are sufficient waypoints.
        
        # Add the start point (Entrance) to the beginning
        # Entrance: [77.6206, 13.1217] (Lon, Lat) -> [13.1217, 77.6206] (Lat, Lon)
        # We should check if navigator knows Entrance coords.
        if start_point in navigator.locations:
            start_coord = navigator.locations[start_point]
            route_coords.insert(0, [start_coord[1], start_coord[0]])
        ...
        emit('navigation_response', {'error': 'Could not find a path.'})
